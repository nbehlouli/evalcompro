...
public class LoginModel
{
    ...
    private List<CompteBean> user_compte;

    private static int profile_id;

    private static int database_id;

    public int checkLoginPwd( String login, String password )
        throws SQLException, ParseException
    {
        PwdCrypt pwdcrypt = new PwdCrypt();
        user_compte = new ArrayList<CompteBean>();
                    ...
                    if ( pwdcrypt.decrypter( rs.getString( "pwd" ) ).trim().equalsIgnoreCase( password.trim() ) )
                    {
                        ...
                        if ( checkLoginValidity( rs.getDate( "val_date_deb" ), rs.getDate( "val_date_fin" ) ) == true )
                        {
                            ...
                            CompteBean db = new CompteBean( rs.getInt( "id_compte" ), rs.getInt( "id_profile" ),
                                                            rs.getString( "login" ), pwdcrypt.decrypter( rs
                                                                .getString( "pwd" ) ), rs.getInt( "database_id" ),
    ...
    public static void checkProfile( List<CompteBean> compte_user )
        throws SQLException
    {

        Iterator<CompteBean> it = compte_user.iterator();
        CreateDatabaseCon dbcon = new CreateDatabaseCon();
        Connection conn = (Connection) dbcon.connectToDB();
        Statement stmt;
        CompteBean cpb;
        ...
        String select_profile = " select id_ecran,code_ecran,libelle_ecran,code_menu,libelle_menu"
            + " from liste_ecran where code_ecran in (" + "select   code_ecran from droits where id_profile=#profile"
        ...
        while ( rs.next() )
        {
            ...
            EcranBean ecranbean = new EcranBean( val, rs.getString( "code_ecran" ), rs.getString( "libelle_ecran" ),
                                                 rs.getString( "code_menu" ), rs.getString( "libelle_menu" ) );
    ...
    public List<CompteBean> getUser_compte()
    {
        return user_compte;
    }

    public void setUser_compte( List<CompteBean> user_compte )
    {
